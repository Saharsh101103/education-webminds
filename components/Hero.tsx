import { ArrowDownIcon } from 'lucide-react'
import Image from 'next/image'
import React from 'react'

function Hero() {
  return (
    <div className='hero min-h-screen max-h-fit   min-w-screen max-w-screen text-white --font-aladin'>
            <div className=  'flex flex-col gap-32 md:gap-52 h-full w-full justify-center md:px-44 py-20 md:py-60 pr-8 md:pr-0 z-10'>
                <div className='flex flex-col px-[10%] w-full gap-8 md:gap-16'>
                    <div className='flex justify-start'><p className='text-4xl'>Online</p></div>
                    <div className='flex justify-center'><p className='text-7xl md:text-9xl font-bold'>Education</p></div>
                    <div className='flex  justify-end'><p>"Welcome to our dynamic e-learning platform, your gateway to knowledge."</p></div>
                </div>
                <div className='px-2 flex flex-col'>

                <div className='flex justify-end'>
                <div className='border-white border-2 flex justify-center items-center md:w-20 md:h-20 w-10 h-10 rounded-full'>
                    <ArrowDownIcon className=' md:scale-150'/>
                </div>
                </div>
                <div className='px-10'>
                    <p className='text-2xl md:text-4xl'>Learn  .  Implement . Repeat</p>
                </div>
                </div>
            </div>
    </div>
  )
}

export default Hero
